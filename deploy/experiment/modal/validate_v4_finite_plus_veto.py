...
def _score_finite(model, tok, test_lines, margin_abstain=0.0, collect=False):
    ...
    for l in test_lines:
        ...
        if d1 > POLICY_MAX_DISTANCE:
            # nearest known schema too far -> genuinely new
            pred = {"decision": "new_candidate", "novelty": "structural"}
        elif (d2 - d1) < POLICY_MIN_MARGIN and best.get("decision") == "match_schema":
            # two candidates near-equidistant -> ambiguous
...
def main(jsonl: str, epochs: int = 3, rank: int = 16):
    ...
    te = te[:120]
    print(f"train={len(tr)} test={len(te)} epochs={epochs} — launching v4 (finite + deterministic vetoes)...")
    r = train_and_eval.remote(tr, te, epochs=epochs, rank=rank)
    with open(jsonl.replace(".jsonl", "") + "_v4_result.json", "w") as f:
